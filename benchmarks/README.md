...
Use `--source file` to compare reads of a generated binary file with the same cache policy:
    ...
    uv run maturin develop --uv --release
    uv run --no-sync python benchmarks/stream.py --sizes-mib 1,8,128 --chunk-kib 4,64,1024
    uv run --no-sync python benchmarks/stream.py --sizes-mib 8 --chunk-kib 64 --source file
